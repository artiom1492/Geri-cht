    ...
    if (bgItems.length) {
        document.addEventListener('windowScroll', function() {
            ...
            bgItems.forEach(bgItem => {
                ...
                let bgItemsPosition = bgItem.getBoundingClientRect().top + scrollY;
                let bgItemsHeight = bgItem.offsetHeight;
                let bgItemBg = bgItem.querySelector('.bg-item');
                let bgItemScrollPrc = Math.abs((bgItem.getBoundingClientRect().top - window.innerHeight) / (bgItemsHeight + window.innerHeight) * 100);
                let bgItemsPositionValue = (bgItemsHeight / 100 * size) / 100 * bgItemScrollPrc;
                if (scrollY > bgItemsPosition - window.innerHeight && scrollY < bgItemsPosition + bgItemsHeight) {
                    bgItemBg.style.cssText = `transform: translate3D(0,${bgItemsPositionValue}px,0)`
                    console.log(bgItemScrollPrc);
